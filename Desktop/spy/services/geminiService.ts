...
export const analyzeLibraryUrl = async (url: string): Promise<UrlAnalysisResult | null> => {
  try {
    ...
    try {
      const response = await callEdgeFunction('analyze-url', { url });
...
export const getTrafficAnalytics = async (
  ...
  try {
    const response = await callEdgeFunction('analyze-traffic', {
      brandName,
      url,
      libraryEntryId,
    });
...
export const getCountryTrends = async (
  ...
  try {
    const response = await callEdgeFunction('research-market', {
      country,
      type: 'country-trends',
    });
    const result = await response.json();
